...
int CSimpleModulus::AddBits(void*lpDest,int iDestBitPos,void*lpSource,int iBitSourcePos,int iBitLen)	// Completed Emulated
{
	...
	memset(pTempBuffer, 0, static_cast<size_t>(iTempBufferLen) + 1);
...
BOOL CSimpleModulus::SaveKey(LPSTR lpszFileName, WORD wFileHeader, BOOL bSaveModulus, BOOL bSaveEncKey, BOOL bSaveDecKey, BOOL bSaveXORKey)
{
	...
	HeaderBuffer.dwSize = (static_cast<size_t>(bSaveModulus) + bSaveEncKey + bSaveDecKey + bSaveXORKey) * sizeof(XORTable) + sizeof(ENCDEC_FILEHEADER);
...
BOOL CSimpleModulus::LoadKey(LPSTR lpszFileName,
                             WORD wFileHeader,
                             BOOL bLoadModulus,
                             BOOL bLoadEncKey,
                             BOOL bLoadDecKey,
                             BOOL bLoadXORKey)
{
    ...
    // Read header
    if (MU_fread(&HeaderBuffer, 1, sizeof(ENCDEC_FILEHEADER), hFile) != sizeof(ENCDEC_FILEHEADER))
    {
        MU_fclose(hFile);
        return FALSE;
    }
    ...
    if (HeaderBuffer.sFileHeader == wFileHeader)
    {
        ...
        if (HeaderBuffer.dwSize == expectedSize)
        {
            if (bLoadModulus)
            {
                MU_fread(XORTable, 1, sizeof(XORTable), hFile);
                for (int n = 0; n < 4; n++)
